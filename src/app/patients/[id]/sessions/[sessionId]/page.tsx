...
function WeekPillar({ week, color }: { week: WeeklyPlan; color: string }) {
  return (
    <div style={{ background: '#fff', borderRadius: 10, border: `2px solid ${color}25`, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ padding: '10px' }}>
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: '#ef4444', textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 4 }}>🔍 Root Cause</div>
          <div style={{ fontSize: 10, color: '#374151', lineHeight: 1.5 }}>{week.cause}</div>
        </div>
        <div>
          <div style={{ fontSize: 9, fontWeight: 700, color, textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 4 }}>✨ Actions</div>
          {(week.actions ?? []).map((a, i) => (
            <div key={i} style={{ fontSize: 10, color: '#374151', marginBottom: 4, display: 'flex', gap: 4, lineHeight: 1.4 }}>
              <span style={{ color, flexShrink: 0, fontWeight: 700 }}>›</span>{a}
            </div>
          ))}
        </div>
      </div>
